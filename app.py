...
<div style="margin:.65rem 0 .35rem 0;padding:.85rem .9rem;border:1px solid rgba(46,204,113,.38);border-radius:16px;background:linear-gradient(135deg,rgba(22,128,58,.16),rgba(17,24,39,.20));">
  ...
  <div style="margin-top:.55rem;font-size:.76rem;font-weight:700;color:#a7f3d0;">{html.escape(saving)}</div>
                ...
                with nav3:
                    if st.button("💡 Dicas", use_container_width=True, key="gm_sidebar_nav_daily_pick"):
...
def render_core_markets_dashboard(a, b, team_a, team_b, df, probs=None, sample_games=0):
    ...
    with tabs[1]:
        ...
        _gm_market_card("⚽ Gols — 1º tempo", first_status,
        ...
        _gm_market_card("⚽ Gols — 2º tempo", second_status,
...
def gm_render_compact_analysis_grid(team_a, team_b, probs, expectations, a, b, df, sample_games=0):
    ...
    st.markdown(
        ...
        .gm-cg-good{border-color:#25a86f;background:linear-gradient(145deg,#0d281f,#123126);box-shadow:inset 0 0 0 1px rgba(39,229,141,.06)}.gm-cg-good b{color:#28df88}
        .gm-cg-hot{border-color:#35f39b;background:linear-gradient(145deg,#0a3525,#105039);box-shadow:inset 0 0 0 1px rgba(39,229,141,.12),0 0 14px rgba(39,229,141,.08)}
        .gm-cg-hot b{color:#39f19a;font-size:1.02rem}.gm-cg-hot span{color:#b8d8cb}
        .gm-cg-soft{opacity:1;border-color:#304454;background:linear-gradient(145deg,#101d2a,#122131)}.gm-cg-soft b{color:#dce6ec}.gm-cg-soft span{color:#9fb0bd}
...
def gm_share_market_sections(team_a, team_b, probs, expectations, a, b, df, sample_games=0):
    ...
    if g:
        ...
        if goal_split:
            for title,key in (("⚽ Gols — 1º tempo","first"),("⚽ Gols — 2º tempo","second")):
                val=goal_split[key]
    ...
    if c:
        ...
        for title,metric,lines in (("⛳ Escanteios — 1º tempo","Escanteios 1T",[2.5,3.5,4.5,5.5,6.5]),("⛳ Escanteios — 2º tempo","Escanteios 2T",[2.5,3.5,4.5,5.5,6.5])):
      ...
      for(const s of secs){{
        ...
        for(let i=0;i<rows.length;i+=2){{
          ...
          pair.forEach((r,j)=>{{
            ...
            else if(pct!==null&&pct>=70&&pct<100){{rr(c,bx,yy,bw,44,10,'#0d281f','#25a86f',1.4);}} else if(pct!==null){{rr(c,bx,yy,bw,44,10,'#122131','#304454',1.1);}}
            const labelColor=isProj?'#63d7a1':C.muted;
            const valColor=isProj?C.text:((pct!==null&&pct>=85&&pct<100)?'#45f5a3':((pct!==null&&pct>=70&&pct<100)?C.green:(pct!==null?'#d7e1e7':C.text)));
...
def render_analysis():
    ...
    # V220: oportunidades continuam calculadas internamente para os motores existentes,
    # mas o bloco redundante no rodapé da análise foi removido. Os próprios mercados
    # acima já recebem destaque proporcional à probabilidade.
...
def gm_daily_pick_source_payload(target_date_iso):
    ...
    # V173: recuperação dirigida SOMENTE para as partidas oficiais que faltaram
    # no lote por data. Em dias pequenos (como a grade atual de 5 jogos), isso
    # evita o falso cenário de centenas de linhas recebidas e zero mercado
    # casável, sem voltar à varredura pesada de partidas alheias à grade GM.
    for mid in missing_prediction_ids:
        try:
            direct, derr = gm_apifootball_request("get_predictions", match_id=mid)
            if not derr and isinstance(direct, list):
                hit = next((r for r in direct if isinstance(r, dict) and str(r.get("match_id") or "").strip() == mid), None)
                if hit is not None:
                    pred_by_match[mid] = hit
                    direct_prediction_hits += 1
        except Exception:
            pass
    for mid in missing_odds_ids:
        try:
            direct, derr = gm_apifootball_request("get_odds", match_id=mid)
            if not derr and isinstance(direct, list):
                hits = [r for r in direct if isinstance(r, dict) and str(r.get("match_id") or "").strip() == mid]
                if hits:
                    odds_by_match.setdefault(mid, []).extend(hits)
                    direct_odds_hits += 1
        except Exception:
            pass
    ...
    result = {
        ...
        "batch_only_fastpath": False,
        "targeted_official_recovery": True,
        "error": source_error,
    }
...
def gm_daily_pick_publish_selected(choice):
    ...
    payload = {
        ...
        "p_model_meta": {
            ...
            **(choice.get("model_meta_extra") or {}),
        },
    }
...
def gm_render_leverage_block(rows=None, auto_generate=False, show_current=True):
    ...
    if current and show_current:
        ...
        st.caption("Ainda não há Alavancagem publicada para hoje.")
...
def gm_render_main_shortcuts():
    ...
    with c2:
        if st.button("💡 Dicas",use_container_width=True,key="gm_home_daily_pick"): st.session_state["gm_main_view"]="daily_pick"; st.rerun()
...
if _gm_main_view == "daily_pick":
    gm_render_tips_hub(is_free=not bool(_gm_caps["current_daily_picks"]))
